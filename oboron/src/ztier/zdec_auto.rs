...
pub(crate) fn dec_any_scheme_ztier(
    ...
    let mut buffer = match crate::dec::decode_obtext_to_payload(obtext, encoding) {
        ...
        Err(decode_err) => {
            ...
            #[cfg(feature = "legacy")]
            {
                ...
                return dec_legacy_fallback(zkeychain, obtext, format).or(Err(decode_err));
            }
    ...
    if buffer.len() < SCHEME_MARKER_SIZE {
        ...
        {
            let format = Format::new(Scheme::Legacy, encoding);
            return dec_legacy_fallback(zkeychain, obtext, format).or(Err(Error::PayloadTooShort));
        }
        #[cfg(not(feature = "legacy"))]
    ...
    if scheme_marker == ZMOCK1_MARKER {
        let plaintext_bytes = decrypt_zmock1(zkeychain.zmock1(), &buffer)?;
        return bytes_to_string(plaintext_bytes);
    }
...
pub(crate) fn dec_any_scheme_c32_ztier(
    ...
    dec_any_scheme_ztier(zkeychain, Encoding::C32, obtext)
}
...
pub(crate) fn dec_any_scheme_b32_ztier(
    ...
    dec_any_scheme_ztier(zkeychain, Encoding::B32, obtext)
}
...
pub(crate) fn dec_any_scheme_b64_ztier(
    ...
    dec_any_scheme_ztier(zkeychain, Encoding::B64, obtext)
}
...
pub(crate) fn dec_any_scheme_hex_ztier(
    ...
    dec_any_scheme_ztier(zkeychain, Encoding::Hex, obtext)
}
...
pub(crate) fn dec_any_format_ztier(zkeychain: &ZKeychain, obtext: &str) -> Result<String, Error> {
    ...
    {
        if let Ok(result) = dec_any_scheme_b64_ztier(zkeychain, obtext) {
            return Ok(result);
        }
    }
    ...
    if obtext.chars().any(|c| c.is_ascii_uppercase()) {
        ...
        if let Ok(result) = dec_any_scheme_b32_ztier(zkeychain, obtext) {
            return Ok(result);
        }
        if let Ok(result) = dec_any_scheme_b64_ztier(zkeychain, obtext) {
            return Ok(result);
    ...
    if obtext.chars().any(|c| c.is_ascii_lowercase() && c > 'f') {
        ...
        if let Ok(result) = dec_any_scheme_c32_ztier(zkeychain, obtext) {
            return Ok(result);
        }
        if let Ok(result) = dec_any_scheme_b64_ztier(zkeychain, obtext) {
            return Ok(result);
    ...
    if let Ok(result) = dec_any_scheme_hex_ztier(zkeychain, obtext) {
        return Ok(result);
    }
    if let Ok(result) = dec_any_scheme_c32_ztier(zkeychain, obtext) {
        return Ok(result);
    }
    dec_any_scheme_b64_ztier(zkeychain, obtext)
}
