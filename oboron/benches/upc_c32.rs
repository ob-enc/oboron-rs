...
fn load_benchmark_specs() -> Vec<BenchmarkSpec> {
    let possible_paths = vec![
        PathBuf::from("benches/benchmarks_upc_c32.jsonl"),
        PathBuf::from("oboron/benches/benchmarks_upc_c32.jsonl"),
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("benches/benchmarks_upc_c32.jsonl"),
    ];

    for path in &possible_paths {
        if path.exists() {
            eprintln!("Found upc.c32 benchmarks at: {:?}", path);
            ...
            let specs: Vec<BenchmarkSpec> = data
                ...
                .collect();
            eprintln!("Loaded {} upc.c32 benchmark specifications", specs.len());
            return specs;
        }
    }

    eprintln!("Warning: benchmarks_upc_c32.jsonl not found");
    vec![]
}
...
fn run_upc_b32_benchmarks(c: &mut Criterion) {
    ...
    if specs.is_empty() {
        eprintln!("No upc.c32 specs loaded");
        return;
    }
    ...
    eprintln!("Registered {} upc.c32 benchmarks", bench_count);
}
