...
use oboron::{Ob00Base32Rfc, Oboron};
...
fn load_benchmark_specs() -> Vec<BenchmarkSpec> {
    let possible_paths = vec![
        PathBuf::from("benches/benchmarks_ob00_b32.jsonl"),
        PathBuf::from("oboron/benches/benchmarks_ob00_b32.jsonl"),
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("benches/benchmarks_ob00_b32.jsonl"),
    ];

    for path in &possible_paths {
        if path.exists() {
            eprintln!("Found ob00:b32 benchmarks at: {:?}", path);
            ...
            let specs: Vec<BenchmarkSpec> = data
                ...
                .collect();
            eprintln!("Loaded {} ob00:b32 benchmark specifications", specs.len());
            return specs;
        }
    }

    eprintln!("Warning: benchmarks_ob00_b32.jsonl not found");
    vec![]
}

fn run_ob00_b32_benchmarks(c: &mut Criterion) {
    let specs = load_benchmark_specs();

    if specs.is_empty() {
        eprintln!("No ob00:b32 specs loaded");
        return;
    }
    ...
    eprintln!("Registered {} ob00:b32 benchmarks", bench_count);
}

criterion_group!(benches, run_ob00_b32_benchmarks);
