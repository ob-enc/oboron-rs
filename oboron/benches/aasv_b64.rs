...
fn load_benchmark_specs() -> Vec<BenchmarkSpec> {
    let possible_paths = vec![
        PathBuf::from("benches/benchmarks_aasv_b64.jsonl"),
        PathBuf::from("oboron/benches/benchmarks_aasv_b64.jsonl"),
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("benches/benchmarks_aasv_b64.jsonl"),
    ];

    for path in &possible_paths {
        if path.exists() {
            eprintln!("Found aasv.b64 benchmarks at: {:?}", path);
            ...
            let specs: Vec<BenchmarkSpec> = data
                ...
                .collect();
            eprintln!("Loaded {} aasv.b64 benchmark specifications", specs.len());
            return specs;
        }
    }

    eprintln!("Warning: benchmarks_aasv_b64.jsonl not found");
    vec![]
}

fn run_aasv_b64_benchmarks(c: &mut Criterion) {
    let specs = load_benchmark_specs();

    if specs.is_empty() {
        eprintln!("No aasv.b64 specs loaded");
        return;
    }

    // Create ob once, OUTSIDE the timed loop
    let ob = AasvB64::new_keyless().unwrap();
    ...
    eprintln!("Registered {} aasv.b64 benchmarks", bench_count);
}

criterion_group!(benches, run_aasv_b64_benchmarks);
