...
use oboron::{AasvB32, ObtextCodec};
...
fn load_benchmark_specs() -> Vec<BenchmarkSpec> {
    let possible_paths = vec![
        PathBuf::from("benches/benchmarks_aasv_b32.jsonl"),
        PathBuf::from("oboron/benches/benchmarks_aasv_b32.jsonl"),
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("benches/benchmarks_aasv_b32.jsonl"),
    ];

    for path in &possible_paths {
        if path.exists() {
            eprintln!("Found aasv.b32 benchmarks at: {:?}", path);
            ...
            let specs: Vec<BenchmarkSpec> = data
                ...
                .collect();
            eprintln!("Loaded {} aasv.b32 benchmark specifications", specs.len());
            return specs;
        }
    }

    eprintln!("Warning: benchmarks_aasv_b32.jsonl not found");
    vec![]
}

fn run_aasv_b32_benchmarks(c: &mut Criterion) {
    let specs = load_benchmark_specs();

    if specs.is_empty() {
        eprintln!("No aasv.b32 specs loaded");
        return;
    }

    // Create ob once, OUTSIDE the timed loop
    let ob = AasvB32::new_keyless().unwrap();
    ...
    eprintln!("Registered {} aasv.b32 benchmarks", bench_count);
}

criterion_group!(benches, run_aasv_b32_benchmarks);
