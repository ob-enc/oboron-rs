...
fn load_benchmark_specs() -> Vec<BenchmarkSpec> {
    let possible_paths = vec![
        PathBuf::from("benches/benchmarks_ob31_c32.jsonl"),
        PathBuf::from("oboron/benches/benchmarks_ob31_c32.jsonl"),
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("benches/benchmarks_ob31_c32.jsonl"),
    ];

    for path in &possible_paths {
        if path.exists() {
            eprintln!("Found ob31:c32 benchmarks at: {:?}", path);
            ...
            let specs: Vec<BenchmarkSpec> = data
                ...
                .collect();
            eprintln!("Loaded {} ob31:c32 benchmark specifications", specs.len());
            return specs;
        }
    }

    eprintln!("Warning: benchmarks_ob31_c32.jsonl not found");
    vec![]
}

fn run_ob31_c32_benchmarks(c: &mut Criterion) {
    let specs = load_benchmark_specs();

    if specs.is_empty() {
        eprintln!("No ob31:c32 specs loaded");
        return;
    }
    ...
    eprintln!("Registered {} ob31:c32 benchmarks", bench_count);
}

criterion_group!(benches, run_ob31_c32_benchmarks);
