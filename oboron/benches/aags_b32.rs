...
use oboron::{AagsB32, ObtextCodec};
...
fn load_benchmark_specs() -> Vec<BenchmarkSpec> {
    let possible_paths = vec![
        PathBuf::from("benches/benchmarks_aags_b32.jsonl"),
        PathBuf::from("oboron/benches/benchmarks_aags_b32.jsonl"),
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("benches/benchmarks_aags_b32.jsonl"),
    ];

    for path in &possible_paths {
        if path.exists() {
            eprintln!("Found aags.b32 benchmarks at: {:?}", path);
            ...
            let specs: Vec<BenchmarkSpec> = data
                ...
                .collect();
            eprintln!("Loaded {} aags.b32 benchmark specifications", specs.len());
            return specs;
        }
    }

    eprintln!("Warning: benchmarks_aags_b32.jsonl not found");
    vec![]
}

fn run_aags_b32_benchmarks(c: &mut Criterion) {
    let specs = load_benchmark_specs();

    if specs.is_empty() {
        eprintln!("No aags.b32 specs loaded");
        return;
    }

    // Create ob once, OUTSIDE the timed loop
    let ob = AagsB32::new_keyless().unwrap();
    ...
    eprintln!("Registered {} aags.b32 benchmarks", bench_count);
}

criterion_group!(benches, run_aags_b32_benchmarks);
