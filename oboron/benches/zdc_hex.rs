...
use oboron::{ObtextCodec, ZfbcxHex};
...
fn load_benchmark_specs() -> Vec<BenchmarkSpec> {
    let possible_paths = vec![
        PathBuf::from("benches/benchmarks_zfbcx_hex.jsonl"),
        PathBuf::from("oboron/benches/benchmarks_zfbcx_hex.jsonl"),
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("benches/benchmarks_zfbcx_hex.jsonl"),
    ];

    for path in &possible_paths {
        if path.exists() {
            eprintln!("Found zfbcx.hex benchmarks at: {:?}", path);
            ...
            let specs: Vec<BenchmarkSpec> = data
                ...
                .collect();
            eprintln!("Loaded {} zfbcx.hex benchmark specifications", specs.len());
            return specs;
        }
    }

    eprintln!("Warning: benchmarks_zfbcx_hex.jsonl not found");
    vec![]
}

fn run_zfbcx_hex_benchmarks(c: &mut Criterion) {
    let specs = load_benchmark_specs();

    if specs.is_empty() {
        eprintln!("No zfbcx.hex specs loaded");
        return;
    }

    // Create ob once, OUTSIDE the timed loop
    let ob = ZfbcxHex::new_keyless().unwrap();
    ...
    eprintln!("Registered {} zfbcx.hex benchmarks", bench_count);
}

criterion_group!(benches, run_zfbcx_hex_benchmarks);
