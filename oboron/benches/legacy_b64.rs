...
use oboron::{LegacyBase64, Oboron};
...
fn load_benchmark_specs() -> Vec<BenchmarkSpec> {
    let possible_paths = vec![
        PathBuf::from("benches/benchmarks_legacy_b64.jsonl"),
        PathBuf::from("oboron/benches/benchmarks_legacy_b64.jsonl"),
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("benches/benchmarks_legacy_b64.jsonl"),
    ];
    ...
    eprintln!("Warning: benchmarks_legacy_b64.jsonl not found");
    vec![]
}

fn run_legacy_b64_benchmarks(c: &mut Criterion) {
    ...
    let ob = LegacyBase64::new_keyless().unwrap();
