...
use oboron::{ApsvHex, ObtextCodec};
...
fn load_benchmark_specs() -> Vec<BenchmarkSpec> {
    let possible_paths = vec![
        PathBuf::from("benches/benchmarks_apsv_hex.jsonl"),
        PathBuf::from("oboron/benches/benchmarks_apsv_hex.jsonl"),
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("benches/benchmarks_apsv_hex.jsonl"),
    ];

    for path in &possible_paths {
        if path.exists() {
            eprintln!("Found apsv_hex benchmarks at: {:?}", path);
            ...
            let specs: Vec<BenchmarkSpec> = data
                ...
                .collect();
            eprintln!("Loaded {} apsv_hex benchmark specifications", specs.len());
            return specs;
        }
    }

    eprintln!("Warning: benchmarks_apsv_hex.jsonl not found");
    vec![]
}

fn run_apsv_hex_benchmarks(c: &mut Criterion) {
    let specs = load_benchmark_specs();

    if specs.is_empty() {
        eprintln!("No apsv_hex specs loaded");
        return;
    }

    // Create ob once, OUTSIDE the timed loop
    let ob = ApsvHex::new_keyless().unwrap();
    ...
    eprintln!("Registered {} apsv_hex benchmarks", bench_count);
}

criterion_group!(benches, run_apsv_hex_benchmarks);
