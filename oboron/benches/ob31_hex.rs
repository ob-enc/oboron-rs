...
fn load_benchmark_specs() -> Vec<BenchmarkSpec> {
    let possible_paths = vec![
        PathBuf::from("benches/benchmarks_ob31_hex.jsonl"),
        PathBuf::from("oboron/benches/benchmarks_ob31_hex.jsonl"),
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("benches/benchmarks_ob31_hex.jsonl"),
    ];

    for path in &possible_paths {
        if path.exists() {
            eprintln!("Found ob31:hex benchmarks at: {:?}", path);
            ...
            let specs: Vec<BenchmarkSpec> = data
                ...
                .collect();
            eprintln!("Loaded {} ob31:hex benchmark specifications", specs.len());
            return specs;
        }
    }

    eprintln!("Warning: benchmarks_ob31_hex.jsonl not found");
    vec![]
}

fn run_ob31_hex_benchmarks(c: &mut Criterion) {
    let specs = load_benchmark_specs();

    if specs.is_empty() {
        eprintln!("No ob31:hex specs loaded");
        return;
    }

    // Create ob once, OUTSIDE the timed loop
    let ob = Ob31Hex::new_keyless().unwrap();
    ...
    eprintln!("Registered {} ob31:hex benchmarks", bench_count);
}

criterion_group!(benches, run_ob31_hex_benchmarks);
